function TreeMap(args){
  ...
  this.svg = d3.select(this.selector).html("").append("svg")
    ...
    .attr("height", this.height+this.padding.top)
  this.viz = this.svg.append("g")
    .attr("class", "viz")
    .data([this.heirarchical_data])
  if(this.total == "true"){
    this.viz.attr("transform", "translate(0, "+this.padding.top+")");
  }
}
...
TreeMap.prototype.add_mouse_events = function(){
  ...
  cell.on("mouseover", function(d){
    ...
    make_mouseover(this, [_this.width, _this.height+_this.padding.top], mouseover_d, _this.padding.top)
