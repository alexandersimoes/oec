function stacked_draw(args){
	...
	$("#tool_pane #layout input").change(function(){
		// console.log(data)
		var da = stacked_value.offset("expand")(std)
		// console.log(stacked_value(std))
		max_y = d3.max(da, function(d) {
			return d3.max(d["years"], function(d) {
				return d.y0 + d.y;
			});
		});
		paths.data(da).attr("d", function(d, i){
			return area(d.years)
		})
	});
...
function stacked_change_layout(){
	d3.selectAll("g.stacked path")
		.transition().ease("bounce").duration(1500)
		.attr("fill", function(d) {
			// return d.meta.community ? d.meta.community.color : d.meta.region.color;
			// return d.meta.category.color
			return "red"
		})
		// .attr("d", function(d, i){
			// return area(d.years)
