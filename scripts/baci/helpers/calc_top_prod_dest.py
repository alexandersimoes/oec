...
def calc_top_prod_dest(yo, yp, yop, yod, hs_id_col):
    ...
    for tf in ['export', 'import']:
        ...
        yo_top_prod_hs4 = this_yop.fillna(0).groupby(level=["origin_id"])["{0}_val".format(tf)].apply(lambda x: x.idxmax()[1])
        
        this_yop = yop.reset_index()
        this_yop = this_yop[this_yop[hs_id_col].str.len() == 8]
        this_yop = this_yop.set_index(["origin_id", hs_id_col])
        yo_top_prod_hs6 = this_yop.fillna(0).groupby(level=["origin_id"])["{0}_val".format(tf)].apply(lambda x: x.idxmax()[1])
        
        yo_top_dest = yod.fillna(0).groupby(level=["origin_id"])["{0}_val".format(tf)].apply(lambda x: x.idxmax()[1])
        
        yo["top_{0}_hs4".format(tf)] = yo_top_prod_hs4
        yo["top_{0}_hs6".format(tf)] = yo_top_prod_hs6
